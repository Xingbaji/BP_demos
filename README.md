...
- 单页结构：简短页头 → 三项最新 Demo → 四项自研引擎 Demo → 十二项公开研究 Demo → 简要归属说明。
...
新增 Latest Demos 区：手物动作重建与物理修正、弹性体重建与杨氏模量估计、全模态仿真。视频和封面位于 `assets/demos/`，保留原分辨率与编码，仅调整 MP4 索引以支持快速起播；点击播放前不加载视频。锅盖素材本身为 0.50× 慢放，播放器按正常速度播放。

内容依据 `/home/pjlab/BP_writer/demo_video/README.md`；来源和校验值见 `MEDIA_SOURCES.md`。
